class Person
  attr_accessor :id, :first_name, :last_name, :address_first_line, :address_second_line, :suburb, :postcode, :phone_number, :email 
  ...
  def self.csv_headers   # useful as a thing when we have to deal with csv to self head , not sure where else
    ...
    # is equiv to below 
    %w[id first_name last_name address_first_line address_second_line suburb postcode phone_number email]
  end
